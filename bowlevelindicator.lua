...
sdk.hook(sdk.get_managed_singleton("snow.StmInputManager"):get_field("_InGameInputDevice"):get_field("_pl_input")
             :get_method("update"), function(args)
    local bow = sdk.to_managed_object(args[2])
    if not bow then return end
    if not type(bow) == sdk.typeof("snow.player.Bow") then return end
                ...
                for i = 0, level - 1 do
                    ...
                    if settings.iconSettings.shape == 2 then
                        draw.filled_rect(x, screenSize.height / 2 - settings.iconSettings.size / 2,
                                         settings.iconSettings.size, settings.iconSettings.size,
...
re.on_draw_ui(function()
    ...
    if configure and imgui.begin_window("Bow Level Indicator Config", true, ImGuiWindowFlags_AlwaysAutoResize) then
        if imgui.tree_node("[Text]") then
            ...
            imgui.text("Position")
            changed, settings.textSettings.location.x = imgui.drag_int("X", settings.textSettings.location.x, 1, 0,
                                                                       screenSize.width)
            if changed then saveConfig() end
            changed, settings.textSettings.location.y = imgui.drag_int("Y", settings.textSettings.location.y, 1, 0,
                                                                       screenSize.height)
            if changed then saveConfig() end
        ...
        if imgui.tree_node("[Icon]") then
            ...
            if changed then saveConfig() end
            changed, settings.iconSettings.offset = imgui.drag_int("Offset", settings.iconSettings.offset, 1,
                                                                   screenSize.height / 2 * -1, screenSize.height / 2)
            if changed then saveConfig() end
